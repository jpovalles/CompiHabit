...
const check_weekly_streak = async (achievement) => {
  let { data, error } = await supabase.rpc("get_max_daily_streak", {
    user_id: achievement.id_user,
  });
...
const check_multiple_habits = async (achievement) => {
  ...
  await update_achievement(
    achievement,
    uniqueHabitTypes.size,
    hasAllCategories,
  );
  return {
    ...achievement,
    current_progress: uniqueHabitTypes.size,
    achieved: hasAllCategories,
  };
};

const check_night_streak = async (achievement) => {
  const { data, error } = await supabase.rpc("get_night_streaks", {
    user_id: achievement.id_user,
  });
  ...
  return {
    ...achievement,
    current_progress: achieved ? 1 : 0,
    achieved: achieved,
  };
};

const check_morning_streak = async (achievement) => {
  const { data, error } = await supabase.rpc("get_morning_streaks", {
    user_id: achievement.id_user,
  });
...
const check_master_consistency = async (achievement) => {
  const { data: badge_data, error: badge_error } = await supabase.rpc(
    "get_max_badge_level",
    { user_id: achievement.id_user },
  );
  ...
  const badgeLevel = badge_data || 0;
  const achieved = badgeLevel >= levels;

  await update_achievement(achievement, badgeLevel, achieved);
  return { ...achievement, current_progress: badgeLevel, achieved: achieved };
};
...
export default async function useCheckAchievements(id_user) {
  ...
  const checkAchievementsProgress = await Promise.all(
    achievements.map(checkAchieved),
  );
